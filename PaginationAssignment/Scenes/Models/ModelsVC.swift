...
final class ModelsVC: BaseVC {
    @IBOutlet private weak var tableView: UITableView! {
        didSet {
            ModelsCellView.register(to: tableView)
    ...
    var viewModel: ModelsMVType!
    var router: ModelsRouterType!

    override func viewDidLoad() {
        super.viewDidLoad()
        title = viewModel.manufacturer.name
...
extension ModelsVC: PagedTableDelegate {
    ...
    func getCell(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: ModelsCellView.reuseIdentifier, for: indexPath) as! ModelsCellView
    ...
    public func didSelectRowAt(_ tableView: UITableView, indexPath: IndexPath) {
        ...
        guard let selectedAutoModel = viewModel.getItem(page: indexPath.section, index: indexPath.row) else {
            return
        }

        router.onModelSelected(manufacturer: viewModel.manufacturer, model: selectedAutoModel)
    }
}
